...
#pragma omp target data map(to:u,w)
  {
...
#pragma omp target teams distribute parallel for map(to:mean) collapse(2)
    ...
    */
    ...
    diff = epsilon;
// #pragma omp target data map(tofrom:diff,iterations)
//       {
    while (epsilon <= diff)
    {
/*
...
#pragma omp target teams distribute parallel for collapse(2)
      ...
      */
      diff = 0.0;
      // #pragma omp target update to(diff)
// #pragma omp target data map(tofrom:diff)
//       {
  // #pragma omp target
  // {
...
//}
      // } // end #pragma omp target data map(tofrom:diff)
  // #pragma omp target update from(diff)
      iterations++;
      ...
      // } // end #pragma omp target data map(tofrom:diff)
  } // end #pragma omp target data map(to:u,w)
